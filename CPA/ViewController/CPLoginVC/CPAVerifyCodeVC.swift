...
class CPAVerifyCodeVC: UIViewController , NVActivityIndicatorViewable  , UITextFieldDelegate {
    ...
    @IBAction func btbBAck_Pressed(_ sender: UIButton) {
...
//        let email = UserDefaults.standard.string(forKey: "email")
        ...
        WebServiceManager.post(params:loginParam as Dictionary<String, AnyObject> , serviceName: VERIFY_CODE, isLoaderShow: true, serviceType: "Login", modelType: UserResponse.self, success: { (response) in
            ...
            if responseObj.success == true {
                let vc = self.storyboard?.instantiateViewController(withIdentifier: "CPAResetPasswordVC") as? CPAResetPasswordVC
                vc?.codeEnter = codeEnter
